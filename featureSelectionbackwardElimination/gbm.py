...
# Train the GBM model on the selected features
gbm_clf = GradientBoostingClassifier()
gbm_clf.fit(X_train_selected_backward_elimination, y_train_resampled)

# Make predictions on the test set using the model with selected features
y_pred = gbm_clf.predict(X_test_selected_backward_elimination)
...
print("Accuracy (GBM with Backward Elimination + SMOTE):", accuracy)
print("Precision (GBM with Backward Elimination + SMOTE):", precision)
print("Recall (GBM with Backward Elimination + SMOTE):", recall)
print("F1-Score (GBM with Backward Elimination + SMOTE):", f1_score)
print("Support (GBM with Backward Elimination + SMOTE):", support)
