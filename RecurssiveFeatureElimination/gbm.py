...
# Initialize RFE with GBM as the estimator
gbm_clf = GradientBoostingClassifier(random_state=42)
rfe = RFE(estimator=gbm_clf, n_features_to_select=10, step=1)
...
gbm_clf.fit(X_train_selected_rfe, y_train_resampled)

# Make predictions on the test set using the model with selected features
y_pred = gbm_clf.predict(X_test_selected_rfe)
...
print("Accuracy (GBM with RFE + SMOTE):", accuracy)
print("Precision (GBM with RFE + SMOTE):", precision)
print("Recall (GBM with RFE + SMOTE):", recall)
print("F1-Score (GBM with RFE + SMOTE):", f1_score)
print("Support (GBM with RFE + SMOTE):", support)
