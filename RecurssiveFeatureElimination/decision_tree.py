...
# Initialize RFE with Decision Tree as the estimator
dt_clf = DecisionTreeClassifier(random_state=42)
rfe = RFE(estimator=dt_clf, n_features_to_select=10, step=1)
...
dt_clf.fit(X_train_selected_rfe, y_train_resampled)

# Make predictions on the test set using the model with selected features
y_pred = dt_clf.predict(X_test_selected_rfe)
...
print("Accuracy (Decision Tree with RFE + SMOTE):", accuracy)
print("Precision (Decision Tree with RFE + SMOTE):", precision)
print("Recall (Decision Tree with RFE + SMOTE):", recall)
print("F1-Score (Decision Tree with RFE + SMOTE):", f1_score)
print("Support (Decision Tree with RFE + SMOTE):", support)
