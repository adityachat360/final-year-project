...
rfe = RFE(estimator=rf_clf, n_features_to_select=10, step=1)
...
rf_clf.fit(X_train_selected_rfe, y_train_resampled)

# Make predictions on the test set using the model with selected features
y_pred = rf_clf.predict(X_test_selected_rfe)
...
print("Accuracy (Random Forest with RFE + SMOTE):", accuracy)
print("Precision (Random Forest with RFE + SMOTE):", precision)
print("Recall (Random Forest with RFE + SMOTE):", recall)
print("F1-Score (Random Forest with RFE + SMOTE):", f1_score)
print("Support (Random Forest with RFE + SMOTE):", support)
