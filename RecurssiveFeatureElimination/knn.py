...
# Initialize RFE with KNN as the estimator
knn_clf = KNeighborsClassifier()
rfe = RFE(estimator=knn_clf, n_features_to_select=10, step=1)
...
knn_clf.fit(X_train_selected_rfe, y_train_resampled)

# Make predictions on the test set using the model with selected features
y_pred = knn_clf.predict(X_test_selected_rfe)
...
print("Accuracy (KNN with RFE + SMOTE):", accuracy)
print("Precision (KNN with RFE + SMOTE):", precision)
print("Recall (KNN with RFE + SMOTE):", recall)
print("F1-Score (KNN with RFE + SMOTE):", f1_score)
print("Support (KNN with RFE + SMOTE):", support)
